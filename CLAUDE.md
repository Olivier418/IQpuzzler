...
- `classes/puzzle.py` — the big one:
  - `Setup`: board + blocks + **all valid placements** (expensive, computed once, shared by reference across every Puzzle/Book built on it). `placements[block_idx]` = flat cell-index array per valid orientation/position. Also owns `print_to_terminal` (ASCII/color board rendering).
  - `State`: mutable solve state (grid + chosen_placement_idx) over a shared `Setup`. `place`/`remove` (checked) vs `place_unchecked`/`remove_unchecked` (used by Solver, no overlap/UNPLACED checks). `.solve()` delegates to `Solver.py`.
  - `Puzzle(State)`: named, has difficulty, optionally pre-filled from a letter grid (`_initialize_grid`, transposes JSON (depth,width) -> internal (width,depth)).
  - `PuzzleBook(UserDict)`: named collection of Puzzles sharing one Setup.
- `classes/game.py` — `Game`: top-level container of `books` (dict name->PuzzleBook), `puzzles` (standalone), `setups` (dict board_key->Setup). One Game per `games/<GameName>/` folder.
